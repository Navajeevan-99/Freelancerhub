    ...
    <header className="navbar">
      ...
      <nav className="nav-links">
        ...
        {auth.user && (
          <div className="nav-profile">
            ...
            <div className="nav-user-details">
              <span className="nav-username">{auth.user.name}</span>
              <span className={`nav-role-tag ${auth.user.role}`}>
                {auth.user.role}
              </span>
            </div>
          </div>
        ...
        <button
          onClick={toggleTheme}
          className="btn-icon"
          style={{ fontSize: "1.1rem", padding: "6px 8px", marginLeft: "12px" }}
          title={theme === "light" ? "Switch to dark mode" : "Switch to light mode"}
        >
          {theme === "light" ? "🌙" : "☀️"}
        </button>
        <button onClick={logout} className="btn-secondary" style={{ marginLeft: "8px", fontSize: "0.85rem", padding: "6px 14px" }}>
          Logout
        </button>
      </nav>
    </header>
